...
public class MaintainApplyDaoImpl implements MaintainApplyDao {
	...
	try {
	    ...
	    sp = (CallableStatement) conn
		    .prepareCall("{CALL baseweb.query_schoolbase()}");
	    sp.execute();
	    rs = sp.getResultSet();
	    while (rs.next()) {
	...
	try {
	    conn = (Connection) SessionFactoryUtils.getDataSource(
		    sessionfactory).getConnection();
	    sp = (CallableStatement) conn
		    .prepareCall("{CALL baseweb.`select_base`()}");
	    sp.execute();
	    rs = sp.getResultSet();
	    while (rs.next()) {
		map = new HashMap<String, String>();
	...
	try {
	    conn = (Connection) SessionFactoryUtils.getDataSource(
		    sessionfactory).getConnection();
	    sp = (CallableStatement) conn
		    .prepareCall("{CALL baseweb.find_basenameFinish(?)}");
	    sp.setString(1, year);
	    sp.execute();
	    rs = sp.getResultSet();
    ...
    public MaintainList query_maintainapply(int offsets, int page, String str,
	...
	try {
	    conn = (Connection) SessionFactoryUtils.getDataSource(
		    sessionfactory).getConnection();
	    sp = (CallableStatement) conn
		    .prepareCall("{CALL baseweb.query_maintainapply(?,?,?,?,?,?)}");
	    sp.setInt(1, offsets);
	    sp.setInt(2, page);
	    sp.setString(3, str);
	    sp.setString(4, str1);
	...
	try {
	    conn = (Connection) SessionFactoryUtils.getDataSource(
		    sessionfactory).getConnection();
	    sp = (CallableStatement) conn
		    .prepareCall("{CALL baseweb.delete_maintainapply(?,?)}");
	    sp.setString(1, str);
	    sp.execute();
	    flag=sp.getInt(2);
	    message=BaseUtils.getException(flag);
	...
	try {
	    conn = (Connection) SessionFactoryUtils.getDataSource(
		    sessionfactory).getConnection();
	    sp = (CallableStatement) conn
		    .prepareCall("{CALL baseweb.export_maintainapply(?,?)}");
	...
	try {
	    conn = (Connection) SessionFactoryUtils.getDataSource(
		    sessionfactory).getConnection();
	    sp = (CallableStatement) conn
		    .prepareCall("{CALL baseweb.`count_maintainyear`()}");
	    sp.execute();
	    rs = sp.getResultSet();
	    while (rs.next()) {
