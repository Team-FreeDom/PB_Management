...
$.ajax({
	...
	success : function(data){
		for(var i=0;i<data[0].length;i++){//获取第一个筛选的基地类别
			...
			$("#baseCategory2ID").after(//获取第二个筛选的基地类别
					"<option class='rest' value="+data[0][i].name+">"+ data[0][i].name + "</option>"
					);
		}
		...
		for(var m=0;m<data[1].length;m++){//获取第一个筛选的学院
			$("#collegeID").after(
			"<option class='rest' value="+data[1][m].college+">"+ data[1][m].college + "</option>"
			);			
			$("#college2ID").after(//获取第二个筛选的学院
					"<option class='rest' value="+data[1][m].college+">"+ data[1][m].college + "</option>"
					);
		}
		/*for(var n=0;n<data[3].length;n++){//获取第二个筛选的学院
			$("#college2ID").after(
			"<option class='rest' value="+data[3][n].base+">"+ data[3][n].base + "</option>"
			);
		}*/
			   ...
			   $.ajax({
				...
				success : function(data){
					for(var i=0;i<data.length;i++){//获取基地名字
						$("#baseNameID").after(
						"<option class='rest' value="+data[i].site+">"+ data[i].site + "</option>"
						);
...
$(document).on("click","#closemodal",function(){
	...
	$.ajax({//获取模态框里面的学院下拉列表
		...
		success : function(data){
			for(var i=0;i<data.length;i++){//获取学院
				$("#selectCollegeID").after("<option class='rest' value="+data[i].dept+">"+ data[i].dept + "</option>");
			}
	...
	/*$("#selectCollege").change(function(){
		...
		$.ajax({//获取模态框里面的学院下拉列表
			...
			success : function(data){
				for(var i=0;i<data.length;i++){//获取专业（第一个）
					$("#selectTeacherID").after("<option class='rest' value="+data[i].name+">"+ data[i].name + "</option>");
				}
