//所有未审核申请记录
  		 ...
  		 "columnDefs" : 
  			[{
  				...
  				"render" : function(data, type, row) {
  					data=row.la_id;
  					//alert(row.userid);
  					return '<input type="checkbox" value="'+ data + '" name="idname" data-id="'+row.userid+'"/>';
  				}
  			}], 					 
  		"language" : {
  ...
  ////////////状态值1：  2：  3：    4：    。。。。。。。
	...
	$('#cancel').click(function() {
		...
		var i=0;
		var userid;////表中缺少此字段，需要补充
		$("input[type='checkbox'][name='iname']:checked").each(function() {
			userid=$(this).data("id");		
			if(i!=0)					
			{
				record_str = record_str+','+$(this).val();
			...
			"aoColumns" : [
					...
					"columnDefs" : 
						[{
							...
							"render" : function(data, type, row) {
								data=row.la_id;
								return '<input type="checkbox" value="'+ data + '" name="iname" data-id="'+row.userid+'"class="ck"  />';
							}
						}], 
					"language" : {
						"lengthMenu" : "每页 _MENU_ 条记录",
